    ...
    <div class="mx-auto max-w-2xl">
        @if ($recentSubmission)
            <div class="mb-4 rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
                <h2 class="mb-2 text-xl font-medium text-gray-900">{{ __('lead::views/public-lead-form.messages.success') }}</h2>
                <dl class="mt-4 grid gap-3 text-sm text-gray-600">
                    <div class="flex justify-between gap-4">
                        <dt class="font-medium text-gray-700">{{ __('lead::filament/resources/lead.fields.name') }}</dt>
                        <dd class="text-right font-medium text-gray-900">{{ $recentSubmission['name'] ?? '—' }}</dd>
                    </div>
                    <div class="flex justify-between gap-4">
                        <dt class="font-medium text-gray-700">{{ __('lead::filament/resources/lead.fields.phone') }}</dt>
                        <dd class="font-mono text-right">{{ $recentSubmission['phone'] ?? '—' }}</dd>
                    </div>
                </dl>
                <div class="mt-4">
                    <a href="{{ route('lead.public.form') }}" class="text-sm font-medium text-blue-600 hover:text-blue-500 hover:underline">
